...
function cKeyboard() {
    $.each(cKeyboard_config.layout, function (e, c) {
        $(cKeyboard_config.target).append('<ul class="cK cKLine"></ul>');
        var a = $(cKeyboard_config.target + " ul")[e];
        $.each(c, function (e, c) {
            $(a).append('<li class="cK ' + c.class + " cKey-" + c.name + '">' + c.text + "</li>");
        });
    }),
        $(cKeyboard_config.target_numeric).hide(),
        $.each(cKeyboard_config.layout_numeric, function (e, c) {
            $(cKeyboard_config.target_numeric).append('<ul class="cK cKLine"></ul>');
            var a = $(cKeyboard_config.target_numeric + " ul")[e];
            $.each(c, function (e, c) {
                $(a).append('<li class="cK ' + c.class + " cKey-" + c.name + '">' + c.text + "</li>");
            });
        }),
        $("body").on(cKeyboard_config.interation_mode, ".cK.cKKey", function () {
            cKeyboard_config.capslock_state
                ? $(cKeyboard_config.input_target).val($(cKeyboard_config.input_target).val() + $(this).html().toUpperCase())
                : $(cKeyboard_config.input_target).val($(cKeyboard_config.input_target).val() + $(this).html());

                $(cKeyboard_config.input_target).trigger("keyup");
        }),
        $("body").on(cKeyboard_config.interation_mode, ".cK.cKFunction.cKey-shift", function () {
            (cKeyboard_config.capslock_state = !cKeyboard_config.capslock_state), cKeyboard_config.capslock_state ? $(".cK.cKKey").addClass("uppercase") : $(".cK.cKKey").removeClass("uppercase");
        }),
